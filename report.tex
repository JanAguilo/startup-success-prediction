...
Six key engineered features were created: \textit{company\_age} (lifespan from founding to closure or last funding), \textit{time\_to\_first\_funding} (duration before securing first investment), \textit{funding\_duration} (active fundraising period length), and \textit{has\_milestones} (binary indicator for milestone achievement). \textit{avg\_funding\_per\_round} was initially created but later removed due to multicollinearity. Log-transformed funding variables were considered but removed to prevent data leakage.
